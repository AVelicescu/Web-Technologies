...
const server = http.createServer((req, res) => {
    ...
    // ------------------------------------------- Animals -------------------------------------------------------------
    if (reqUrl.pathname === '/animals/' && req.method === 'POST') {
        let body = '';
        req.on('data', (chunk) => {
            body += chunk;
        });
        req.on('end', async () => {
            const email = body;
            try {
                connection.query('SELECT * FROM animal ', [], (error, results, fields) => {
                    if (error) {
                        res.writeHead(500, {'Content-Type': 'text/plain'});
                        res.end('Internal server error');
                    }
    ...
    // ------------------------------------------- Animal --------------------------------------------------------------
    if (reqUrl.pathname.startsWith('/animal/') && req.method === 'POST') {
        const animalId = reqUrl.pathname.split('/')[2];
        let body = '';
        req.on('data', (chunk) => {
            body += chunk;
        });
        ...
        try {
            connection.query('SELECT * FROM animal WHERE id = ?', [animalId], (error, results, fields) => {
                if (error) {
                    res.writeHead(500, {'Content-Type': 'text/plain'});
                    res.end('Internal server error');
                }
                if (results.length === 0) {
                    ...
                    const animal = results[0]; // Assuming there is only one animal with the given ID
                    res.writeHead(200, {'Content-Type': 'application/json'});
    ...
    if (reqUrl.pathname.startsWith('/search/') && req.method === 'POST') {
        ...
        try {
            const query = 'SELECT * FROM animal WHERE name LIKE ? OR description LIKE ?';
    ...
    //---------------------------------------- Admin -------------------------------------------------------------------
    if (reqUrl.pathname === '/admin/animals/' && req.method === 'POST') {
        let body = '';
        req.on('data', (chunk) => {
            body += chunk;
        });
    ...
    if (reqUrl.pathname === '/admin/deleteAnimal/' && req.method === 'DELETE') {
        let body = '';
        req.on('data', (chunk) => {
            body += chunk;
        });
        req.on('end', async () => {
            ...
            console.log(id);
            try {
                connection.query('DELETE FROM animal WHERE id = ?', [id], async (error, results, fields) => {
                    if (error) {
                        res.writeHead(500, {'Content-Type': 'text/plain'});
                        res.end('Internal server error');
                    } else {
                        res.writeHead(200, {'Content-Type': 'application/json'});
                        res.end("Successfully deleted the animal");
                    }
                });
            } catch (error) {
                res.writeHead(500, {'Content-Type': 'text/plain'});
    ...
    if (reqUrl.pathname === '/admin/editAnimal/' && req.method === 'PUT') {
        let body = '';
        req.on('data', (chunk) => {
            body += chunk;
        });
        req.on('end', async () => {
            ...
            const photo = requestData.photo;
            const description = requestData.description;
            const category = requestData.category;
            console.log(requestData);
            try {
                connection.query('UPDATE animal SET name = ?, photo = ?, description = ?, category = ? WHERE id = ?', [name, photo, description, category, id], async (error, results, fields) => {
                    if (error) {
                        res.writeHead(500, {'Content-Type': 'text/plain'});
                        res.end('Internal server error');
                    } else {
    ...
    if (reqUrl.pathname === '/admin/addAnimal/' && req.method === 'POST') {
        let body = '';
        req.on('data', (chunk) => {
            body += chunk;
        });
        req.on('end', async () => {
            ...
            console.log(requestData);
            try {
                connection.query('INSERT INTO animal (name, photo, description, category) VALUES (?, ?, ?, ?)', [name, photo, description, category], async (error, results, fields) => {
                    if (error) {
                        res.writeHead(500, {'Content-Type': 'text/plain'});
                        res.end('Internal server error');
                    } else {
                        res.writeHead(200, {'Content-Type': 'application/json'});
                        res.end("Successfully added the animal");
                    }
                });
            } catch (error) {
                res.writeHead(500, {'Content-Type': 'text/plain'});
